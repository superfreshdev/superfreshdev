...
/* ------------------------------------------------------------------------------ */
  ...
  function create_layout_it_doc_card_container_header( dataHeader ) {
    ...
    // Parent Header
    const newHeader = get_new_headerClass( "my-it-docs-card-container-header");

      // create dom
      var newTitleElement = createDivClass( "my-it-docs-card-container-header-title" );

      // create doms
      var imgTitle = get_new_imgClass( dataHeader.img_filename, dataHeader.img_css_class ) ;
      var textTitle = get_new_divText( dataHeader.title )
      ...
      for( let i=0; i < dataCardList.length; i++ ) {
        ...
        switch( dataCardList[i].status_text ) {
          ...
          case "In Progress":
            ...
            var newInProgressElement = get_new_divClassText( string_class_docs_in_progress_element, dataCardList[i].status_text );
          ...
          case "Coming Soon":
            ...
            var newComingSoonElement = get_new_divClassText( string_class_docs_coming_soon_element, dataCardList[i].status_text );
          ...
          case "Actually No Priority":
            ...
            var newActuallyNoPriorityElement = get_new_divClassText( string_class_docs_acutally_no_priority_element, dataCardList[i].status_text );
        ...
        var newImgItDocCard = get_new_imgClass( imgPath, string_class_docs_card_img )
        ...
        var newDocLink = get_new_link_by_onlyPath( dataCardList[i].pdf_url, "Show PDF🐸" )
  ...
  async function get_data_from_it_doc_cards( jsonFile , settedIndex ) {
    ...
    try {
          ...
          if( data_doc_category != -1 ) {
            ...
            delete_domElements_from( "."+string_class_docs_card_container, add_to_it_docs_content_list )
